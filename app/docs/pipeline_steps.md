...
### Текстовый формат ячейки `@` в шаблоне

В Excel для столбцов с кодами, штрихкодами и ведущими нулями задайте в **строке-образце** (`table_template_row`) формат ячейки **Текст** (в Excel это `number_format: @`):

1. Откройте шаблон `.xlsx`, выделите ячейки столбца (например B9 — строка таблицы).
2. `Ctrl+1` → категория **Текст** → ОК (в ячейке можно сразу ввести плейсхолдер `{{row.Штрихкод}}`).
3. Укажите ту же строку в `table_template_row` — оформление и формат `@` копируются на все строки вывода.

**Поведение:**

| `normalize_values` | Тип в DF | Формат `@` в шаблоне | Результат |
|------------------|----------|----------------------|-----------|
| `off` (по умолчанию) | `str` | любой | Текст как в DF, формат ячейки `@` |
| `off` | `int` / `float` | любой | Число, формат из шаблона (например `0.00`) |
| `on` | `str` `604550000383262` | нет | Может стать числом (риск `6,0455E+14`) |
| `on` | `str` | **да (`@`)** | Текст без преобразования в число |
| `on` | `str` `_код`, `00123`, ≥11 цифр | нет | Текст (эвристика идентификаторов) |

Рекомендация: колонки-идентификаторы загружайте как текст (`load_excel` → `dtype: str`) и оставляйте `normalize_values: off`. Для смешанных шаблонов с явными числами включите `normalize_values: on` только там, где нужно.

```yaml
table_start_row: 9
table_template_row: 9   # строка 9 в шаблоне — образец; в колонке B формат «Текст»
table_columns:
  - { df_col: "Наименование", excel_col: 2 }
  - { df_col: "Штрихкод", excel_col: 3 }   # в шаблоне col 3 = @
normalize_values: off
```
...
| `normalize_values` | `on` / `off` (по умолчанию **off**). Если **off** — в ячейку пишется значение **как в DataFrame**: текст (`str`) всегда как текст, числа — как числа; эвристики преобразования строк в число нет. Если **on** — строки вида `42`, `34,55` могут стать числом Excel (удобно для расчётов; длинные коды и `@` см. ниже). |
