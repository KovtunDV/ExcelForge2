...
Параметры шага задаются в YAML в поле `params` выбранного шага в Builder.

Общие правила:

- Имена датафреймов (`dataframe`, `source_df`, `target_df`, …) — строки; DF хранятся в общем контексте выполнения.
...
|----------|-----|----------|
| `input_mode` | строка | `file` — один файл (`file_path`); `mask` — все файлы по маске в `directory`; `latest` — один самый новый по дате изменения среди совпавших с маской. |
...
Диалоги работают только при запуске из **GUI** (Builder / Runner): в контекст подключаются `tk_askopenfilename` и `tk_askdirectory`.
